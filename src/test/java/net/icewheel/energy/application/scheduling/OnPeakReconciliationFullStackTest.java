...
@Disabled
class OnPeakReconciliationFullStackTest {
	...
	@TestConfiguration
	static class Config {
		private static final Instant NOW = Instant.parse("2025-08-18T12:00:00Z"); // 8:00 AM EDT
	...
	static void registerTeslaApiBaseUrl(DynamicPropertyRegistry registry) {
		...
		registry.add("tesla.api-base-url", () -> mockUrl);
		registry.add("tesla.token-url", () -> mockUrl + "oauth2/v3/token");
	}
	...
	@Test
	@DisplayName("Given a missed 7 AM trigger, reconciler calls Tesla API with valid JWT to set on-peak backup")
	void reconciler_whenOnPeakTriggerIsMissed_sendsCorrectApiCall() throws Exception {
		...
		// And: The API shows the wrong state (80%)
		mockWebServer.enqueue(new MockResponse().setResponseCode(200)
				.setHeader(HttpHeaders.CONTENT_TYPE, MediaType.APPLICATION_JSON_VALUE)
				.setBody(readJsonFile("json/site_info_response_80_percent.json")));
		...
		assertThat(postRequest.getBody().readUtf8()).contains("\"backup_percent\":20");
	}
