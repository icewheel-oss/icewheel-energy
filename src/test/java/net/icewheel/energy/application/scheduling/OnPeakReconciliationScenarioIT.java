...
/**
 * Integration test for the {@link PowerwallStateReconciler} focused on the on-peak scenario.
 * This test verifies that if a scheduled 7 AM on-peak trigger is missed, the reconciler will
 * self-heal the system by correcting the Powerwall's state.
 */
@SpringBootTest
@DirtiesContext(classMode = DirtiesContext.ClassMode.AFTER_EACH_TEST_METHOD)
class OnPeakReconciliationScenarioIT {
	 ...
	 */
	@TestConfiguration
	static class OnPeakTestConfig {
		@Bean
		@Primary
		public Clock testClock() {
			// Set time to Monday 8:00 AM, one hour after the on-peak period should have started.
	...
	@Test
	@DisplayName("Given a missed 7 AM on-peak trigger, reconciler should correct state to on-peak level")
	void reconciler_whenOnPeakTriggerIsMissed_correctsState() {
		// Given: A sample utility schedule exists for the weekday.
		int expectedOnPeakPercent = 20;
		int incorrectOffPeakPercent = 80;
		createSampleUtilitySchedule(expectedOnPeakPercent, incorrectOffPeakPercent);

		// And: The Powerwall is still at the old off-peak setting, simulating a missed trigger.
		when(teslaEnergyService.getBackupReservePercent(eq(testUser.getId()), anyString())).thenReturn(incorrectOffPeakPercent);
